...
export function registerCodeModeTools(server: McpServer, catalog: Operation[]): void {
  ...
  server.tool(
    "airtable_search",
    `Search for available Airtable operations by keyword. Returns matching operations from a catalog of ${catalog.length} tools. Use detail="detailed" to see parameter schemas when you're ready to write code for the airtable_execute tool.`,
    {
      query: z
        .string()
  ...
  server.tool(
    "airtable_execute",
    `Execute JavaScript code that orchestrates Airtable API calls. The code runs in a sandbox with callTool(name, params) as the only callable. Use await callTool("operation_name", { ...params }) to call operations found via airtable_search. Must use return to produce a result.
